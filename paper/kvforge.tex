...
We demonstrate that standard LoRA fine-tuning exhibits catastrophic perplexity collapse under long-context inference -- perplexity exceeding 12,000 at 1,024 tokens while the base model achieves PPL $\approx 24$ with LoRA disabled. This collapse has been independently documented by LongLoRA~\cite{longlora} even at rank~256. We treat this not as a bug but as an \textbf{architectural signal}: base-only prefill is not merely a speed optimization -- it is a stability requirement for long-context deployment.
...
We introduce progressive activation schedules that control which subset of LoRA modules is active during each training step. Three schedules were tested:
...
We evaluated GPT-2 Small (124.4M parameters) with LoRA rank~8 injected into 24 attention modules (12 layers $\times$ $c\_attn + c\_proj$). All 7/7 tests pass with identical perplexity (265.06):
...
where $T_p$ is prefill time and $T_d$ is decode time per token. For $N=50$, this yields 2.0$\times$ total throughput improvement. We verified this empirically on GPT-2 Small and Qwen2.5-0.5B using both FP16 and 4-bit compressed caches.
