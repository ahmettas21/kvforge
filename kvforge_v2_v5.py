"""
KVForge v2 — GPT-Neo 1.3B (P100 16GB, v5 fix — fp32 + autocast + GradScaler)
================================================================================
- Model fp32'de kalır (5.5GB)
- Forward: autocast('cuda') ile fp16'da (memory tasarrufu)
- Backward: fp32 gradyan (GradScaler ile uyumlu)
- seq_len=256, batch=1, grad_accum=8
- seed_encoder hidden=512
"""
...
# ============================================================
# HÜCRE 2: MODEL (fp32 + autocast forward)
# ============================================================
import torch
...
# ============================================================
# HÜCRE 4: EĞİTİM (fp32 + autocast + GradScaler)
# ============================================================
def train():
    print("=" * 60)
    print("KVForge v2 — GPT-Neo 1.3B (v5 fp32+autocast)")
    print("=" * 60)
    ...
    print(f"  Using GradScaler (dynamic loss scaling)")
    ...
    # GradScaler — dinamik loss scaling, underflow/overflow'u otomatik yönetir
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for bidx, batch in enumerate(train_loader):
            ...
            # Forward: autocast ile fp16'da (memory tasarrufu)
            # Model fp32'de kalır, sadece forward fp16'da koşar
            with torch.amp.autocast('cuda'):
                loss, _, recon = model(batch, labels=batch, return_seed=True)
            loss = loss / GRAD_ACCUM

            # Backward: GradScaler ile (fp32 gradyanlar)
            ...
            if (bidx + 1) % GRAD_ACCUM == 0:
                ...
                if global_step % 10 == 0:
                    ...
                    nan_flag = " ⚠️NAN" if math.isnan(lv) else ""
                    print(f"E{epoch}|S{global_step:4d}/{total_steps} "
                          f"L={lv:.4f} R={rv:.5f} Mem={mem:.1f}GiB ({elapsed:.0f}s){nan_flag}")
    ...
    results = {
        'model': 'KVForge v2 GPT-Neo 1.3B (v5 fp32+autocast)',
        'config': {
