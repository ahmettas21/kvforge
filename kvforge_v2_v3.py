"""
KVForge v2 — GPT-Neo 1.3B (P100 16GB, v3 fix)
===================================================
- seq_len=256 (önemli: 512'de OOM)
- Gradient checkpointing YOK (P100'de checkpoint + seed encoder OOM yapıyor)
- seed_encoder hidden=512 (2048 yerine, memory tasarrufu)
- batch=1, grad_accum=8 → eff batch=8
"""
...
class ContextualSeedAttention(nn.Module):
    """Düşük memory — seed_encoder hidden=512."""
    ...
    def __init__(self, config):
        ...
        # Düşük memory seed encoder: 4*2048=8192 → 512 → 64
...
def train():
    print("=" * 60)
    print("KVForge v2 — GPT-Neo 1.3B (P100 v3 fix)")
    print("=" * 60)
    ...
    SEQ_LEN = 256           # 256 → P100'de rahat
    BATCH_SIZE = 1
    LR = 1e-4
    NUM_EPOCHS = 3
    GRAD_ACCUM = 8          # eff batch = 8
    MAX_STEPS = 1000

    print(f"\n  Seq: {SEQ_LEN}, Batch: {BATCH_SIZE}, Accum: {GRAD_ACCUM}, Eff: {BATCH_SIZE*GRAD_ACCUM}")
    print(f"  Seed encoder hidden: 512 (low memory)")
    print(f"  Checkpointing: disabled (direct forward)\n")
    ...
    compressed_bits = 2 * config['hidden_size']  # 4096
    seed_bits = config['seed_dim']                # 64
    ...
    results = {
        'model': 'KVForge v2 GPT-Neo 1.3B (v3 fix)',
        'config': {
