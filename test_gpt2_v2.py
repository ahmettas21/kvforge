"""KALoRA — GPT-2 her layer'a ayrı restore head"""
...
print("="*55)
print("KALoRA — GPT-2 Per-Layer Restoration")
print("="*55)
...
t0=time.time()
with torch.no_grad():
    out_n=model.generate(**ids, max_new_tokens=25, do_sample=True, temperature=0.8,
                          pad_token_id=tok.eos_token_id, use_cache=True)
normal=tok.decode(out_n[0], skip_special_tokens=True)
print(f"  \"{normal}\" | {time.time()-t0:.1f}s")

# ── KALoRA — her layer'a AYRI head ──
print("\n[2/3] KALoRA (per-layer heads)...")
with torch.no_grad():
    out=model(input_ids=ids['input_ids'], use_cache=True)

# Her layer için ayrı restore head eğit
heads=[]
total_params=0
for i in range(n_layers):
    with torch.no_grad():
        k,_=get_kv(out.past_key_values, i)
    ...
    total_params+=p
    heads.append(rh)
    print(f"  Layer {i:2d}: MSE {mse_b:.4f}->{mse_a:.4f} ({imp:.1f}% gain) | {p} params")

print(f"  Total restore params: {total_params} ({total_params/442368*100:.1f}% of LoRA)")
print(f"  CR: {cr:.1f}x")
...
with torch.no_grad():
    for i in range(n_layers):
        ...
        kr=k_c.reshape(-1,head_dim)+heads[i](k_c.reshape(-1,head_dim))
        kalora_cache.update(kr.reshape(k.shape).half(), v.half(), i)

torch.manual_seed(42)
t0=time.time()
with torch.no_grad():
    out_k=model.generate(input_ids=ids['input_ids'], max_new_tokens=25, do_sample=True,
...
print(f"  CR: {cr:.1f}x | Toplam restore params: {total_params} ({total_params/442368*100:.1f}% of LoRA)")
print(f"  {'✅ Her layer restore edildi!' if True else ''}")
print(f"{'='*55}")
