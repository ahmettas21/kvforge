...
class NanoSeedAttention(nn.Module):
    ...
    def forward(self, x, return_seed=False):
        ...
        # Causal mask: future token'ları gizle
        # [ÖNEMLİ] Bu mask olmazsa model eğitimde gelecek token'ları görür
        # generation kalitesi düşer, PPL yapay olarak iyimser çıkar
        ...
        if return_seed:
            ...
            # Not: kv_concat.detach() — seed encoder'ın gradyanını keser.
            # Bu, encoder'ın recon loss'u最小ize etmek için K,V orijinallerini
            # bozmasını engeller. İyi bir tasarım tercihi.
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for bidx, batch in enumerate(train_loader):
            ...
            if bidx % 10 == 0:
                ...
                # Gerçek PPL = exp(CE_loss), recon loss dahil DEĞİL
                # Recon loss'u çıkararak PPL hesaplamak yanıltıcı olur
                ce_loss_estimate = avg_loss - RECON_LAMBDA * avg_recon
                ppl = math.exp(min(ce_loss_estimate, 20))
                elapsed = time.time() - t_start
                rate = n / (time.time() - epoch_start) if (time.time() - epoch_start) > 0 else 0
                lr = scheduler.get_last_lr()[0]
                
                print(f"E{epoch}|B{bidx:3d}/{len(train_loader)} "
                      f"L={avg_loss:.4f} CE={ce_loss_estimate:.4f} "
                      f"R={avg_recon:.5f} PPL={ppl:.1f} "
                      f"LR={lr:.2e} {rate:.2f}s/s")
        
        # Validation
        val_loss, val_recon = evaluate(model, val_loader)
        # [DÜZELTİLDİ] Gerçek PPL: sadece CE loss üzerinden, recon loss dahil DEĞİL
        # Validation loss'un içinde zaten recon_lambda * recon var,
        # onu çıkararak gerçek dil modeli performansını görürüz
        val_ce_only = val_loss - RECON_LAMBDA * val_recon
        val_ppl = math.exp(min(val_ce_only, 20))
        
        epoch_time = time.time() - t_start
        print(f"\n  → Epoch {epoch+1} ({epoch_time:.0f}s): "
              f"Train L={avg_loss:.4f} (CE={ce_loss_estimate:.4f}) PPL={ppl:.1f} | "
              f"Val L={val_loss:.4f} (CE={val_ce_only:.4f}) PPL={val_ppl:.1f} Recon={val_recon:.5f}")
