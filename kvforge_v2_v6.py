"""
KVForge v2 — GPT-Neo 1.3B (P100 16GB, v6 — Adafactor)
===========================================================
- Model fp32'de kalır, forward autocast(fp16), backward GradScaler
- Adafactor optimizer — NO momentum/exp_avg, çok daha az memory
...
# ============================================================
# HÜCRE 4: EĞİTİM (Adafactor + fp32 + autocast + GradScaler)
# ============================================================
def train():
    print("=" * 60)
    print("KVForge v2 — GPT-Neo 1.3B (v6 Adafactor)")
    print("=" * 60)
    ...
    print(f"  Optimizer: Adafactor (no momentum, low memory)")
    print(f"  Model: fp32, Forward: autocast(fp16), Backward: GradScaler\n")
    ...
    # Adafactor — no momentum state, sadece variance (RMS) tutar
    # Memory: 1 state per param (adamw'de 2) → ~%50 daha az optimizer memory
    from transformers.optimization import Adafactor
    opt = Adafactor(
        model.parameters(),
        lr=LR,
        weight_decay=0.01,
        scale_parameter=True,      # Adafactor'ün önerilen ayarı
        relative_step=False,       # False = sabit LR kullan
        warmup_init=False,
    )
        ...
        for bidx, batch in enumerate(train_loader):
            ...
            if (bidx + 1) % GRAD_ACCUM == 0:
                ...
                if global_step % 10 == 0:
                    ...
                    rv = recon if recon is not None else 0.0
                    nan_f = " ⚠️NAN" if math.isnan(lv) else ""
                    print(f"E{epoch}|S{global_step:4d}/{total_steps} "
                          f"L={lv:.4f} R={rv:.5f} Mem={mem:.1f}GiB ({elapsed:.0f}s){nan_f}")
    ...
    results = {
        'model': 'KVForge v2 GPT-Neo 1.3B (v6 Adafactor)',
        'config': {
            'layers': 24, 'heads': 16, 'hidden': 2048, 'seed_dim': 64,
            'seed_encoder_hidden': 512,
            'compression': '64×',
            'optimizer': 'Adafactor',
            'dtype': 'fp32+autocast',
            'seq_len': SEQ_LEN, 'batch_size': BATCH_SIZE,
            'grad_accum': GRAD_ACCUM, 'eff_batch': 8,
            'max_steps': min(MAX_STEPS, global_step), 'lr': LR,
        },
        'results': {
