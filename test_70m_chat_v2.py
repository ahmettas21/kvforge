"""KALoRA — Pythia-70m chat testi (clean)"""
...
def get_kv(past, i):
    ...
    for attr in ['to_tuple', 'to_tuple']:
        if hasattr(past, attr):
            t = past.to_tuple()
            return t[i][0].float(), t[i][1].float()
    raise RuntimeError(f"Unknown cache: {type(past)}")

print("="*55)
print("KALoRA — Pythia-70m Chat Testi")
print("="*55)
...
with torch.no_grad():
    restored=kf+rh(kf)
    mse_a=F.mse_loss(restored,k_full).item()
    imp=max(0,(1-mse_a/mse_b)*100)
print(f"  Layer 0: MSE {mse_b:.4f}->{mse_a:.4f} ({imp:.1f}%) | CR={cr:.1f}x")

# Compress + restore ALL layers, then generate
compressed_past=[]
with torch.no_grad():
    for i in range(n_layers):
        k,v=get_kv(out.past_key_values, i)
        k_c,_=svd_compress(k, rank=4)
        # Restore with trained head
        kr=k_c.reshape(-1,head_dim) + rh(k_c.reshape(-1,head_dim))
        kr=kr.reshape(k.shape)
        compressed_past.append((kr.half(), v.half()))

# Generate with compressed KV
torch.manual_seed(42)
t0=time.time()
with torch.no_grad():
    out_k=model.generate(input_ids=input_ids, max_new_tokens=20, do_sample=True,
...
print(f"  {'✅ Çıktılar benzer!' if normal.split()[:5]==kalora.split()[:5] else '⚠️ Farklı çıktılar'}")
print(f"{'='*55}")
